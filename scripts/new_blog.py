...
class FileWorker:
    def __init__(
        self, path, title_id, title, subtitle, author, catalog,
        ...
        if not path or not title_id or not title:
            ...
            raise KeyError("缺少必要信息, 请检查-p, -t参数是否传递")
        self.path = path
    ...
    def wirte_file(self):
        ...
        with open(self.path / f"posts/{self.file_name}", "w+") as f:
            f.writelines(header)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-s", "--subtitle", type=str, default="文章暂存")
    parser.add_argument("-a", "--author", type=str, default="systemime")
    parser.add_argument("-c", "--catalog", type=bool, default=True)
    parser.add_argument("-ts", "--tags", type=str, default="python", help="以,分割")
    parser.add_argument("-d", "--description", type=str, default="", help="文章摘要")
    parser.add_argument(
      "-p", "--path", type=str, default=Path(__file__).resolve(strict=True).parent / "blog", help="博客路径")
